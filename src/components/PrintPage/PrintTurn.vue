<template>
	<div id="printTurn" class="">
		<div id="print" region="center" title="" style="width: 254px">
			<div v-for="order in orderList" :key="order.processOrderId" class="home_info">
				<div class="print_info">
					...
					<div class="order_info">
						<div class="worker_name">
							交接单（来自{{ getUserInfo.department }}:{{ getUserInfo.name }}）
						</div>
						...
						<div class="quantity">
							{{ order.custmoer_code }} &nbsp; {{ order.MONumber }} &nbsp; #
							{{ order.numbrer }}
							&nbsp;
						</div>
						<div class="quantity">订单需求数量：{{ order.quantity }}</div>
						<div class="quantity">订单需求重量：{{ order.weight }}</div>
						<div class="quantity">转移数量：{{ order.Qty }}</div>
						<div class="quantity">转移重量：{{ order.weight }}</div>
						<div class="remark no-wrap">备注：{{ order.remark }}</div>
					</div>
					<div class="user_info">
...
<script setup>
	...
	const print = (data) => {
		...
		// 	.getDetail([15])
		// 	.then((res) => {
		// 		if (res.worklist.length) {
		// 			console.log(res);
		// 			// order = res.worklist[0];
		// 		} else {
		// 			useMessage('未查询到数据', 'warning');
		// 		}
		// 	})
		// 	.finally(() => {
